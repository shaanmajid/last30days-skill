...
| X / Twitter | one of: `AUTH_TOKEN` + `CT0` (explicit cookie values), `XAI_API_KEY`, `SCRAPECREATORS_API_KEY`, or opt-in `FROM_BROWSER` (local browser cookie read) | X items in results | cookie-jar / Bird = free; xAI / ScrapeCreators = paid |
...
# OR explicit local browser cookie scan after consent
